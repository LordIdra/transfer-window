...
pub fn newton_raphson_to_find_stationary_point(
    function: &impl Fn(f64) -> f64,
    starting_x: f64,
    derivative_delta: f64,
    max_delta: f64,
...
#[cfg(test)]
mod test {
    use crate::numerical_methods::newton_raphson::{
        newton_raphson, newton_raphson_to_find_stationary_point,
    };
    ...
    fn test_newton_raphson() {
        ...
        let starting_x = 4.0;
        assert!(
            (newton_raphson(&function, starting_x, 1.0e-4, 1.0e-6, 20).unwrap() - 2.0).abs()
                < 1.0e-3
        );
    }
    ...
    fn test_newton_raphson_to_find_stationary_point() {
        ...
        let starting_x = -1.74;
        assert!(
            (newton_raphson_to_find_stationary_point(&function, starting_x, 1.0e-4, 1.0e-6, 20)
                .unwrap()
                - 2.0)
                .abs()
                < 1.0e-3
