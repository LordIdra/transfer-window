...
/// Returns (value, first derivative, second derivative, third derivative)
pub fn differentiate_3_option_mut(f: &mut impl FnMut(f64) -> Option<f64>, x: f64, delta: f64) -> Option<(f64, f64, f64, f64)> {
    #[cfg(feature = "profiling")]
    let _span = tracy_client::span!("Differentiate 3");
    let f_1 = f(x - delta)?;
    let f_2 = f(x)?;
    ...
    Some((f_2, f_prime_1, f_prime_prime_1, f_prime_prime_prime))
}