...
fn exit_check(model: &model, can_exit: &HashSet<Entity>, entity: Entity) -> Option<Entity> {
    ...
    if can_exit.contains(&parent) {
        let sphere_of_influence = model.get_trajectory_component(parent).get_end_segment().as_orbit().get_sphere_of_influence();
        let distance = position.magnitude();
        if can_exit.contains(&parent) && distance > sphere_of_influence {
            return Some(model.get_trajectory_component(parent).get_end_segment().get_parent());
        }
    }
    None
}

fn entrance_check(model: &model, can_enter: &HashSet<Entity>, entity: Entity) -> Option<Entity> {
    ...
    for sibling in siblings {
        let trajectory_component = model.get_trajectory_component(sibling);
        let end_segment = trajectory_component.get_end_segment();
...
pub fn incremental_find_next_encounter(model: &mut model, start_time: f64, end_time: f64, time_step: f64) -> Option<Encounter> {
    let mut time = start_time;
    let entities = model.get_entities(vec![ComponentType::TrajectoryComponent]);
    while time < end_time {
        for entity in &entities {
            if let Some(new_parent) = entrance_check(model, &entities, *entity) {
                return Some(Encounter::new(EncounterType::Entrance, *entity, new_parent, time));
            }
            if let Some(new_parent) = exit_check(model, &entities, *entity) {
...
mod test {
    ...
    fn test_incremental() {
        ...
        let sun = model.allocate(EntityBuilder::new()
        ...
        let earth = model.allocate(EntityBuilder::new()
        ...
        let moon = model.allocate(EntityBuilder::new()
        ...
        while let Some(encounter) = incremental_find_next_encounter(&mut model, start_time, end_time, 60.0) {
            start_time = encounter.get_time();
            apply_encounter(&mut model, encounter);
        }

        let end_point = model.get_trajectory_component(moon).get_end_segment().as_orbit().get_end_point();
        ...
        let end_point = model.get_trajectory_component(earth).get_end_segment().as_orbit().get_end_point();
