...
use self::entrance_solver::solve_for_entrance;
use self::exit_solver::solve_for_exit;
...
/// It's expected that candidates only contains entities with an orbitable
/// component, and that entity has a path component
...
/// Solves for the next encounter of a single entity by combining the entrance
/// and exit solvers
// - We create an ordered list of all the windows
// - We continually evaluate the soonest window for encounters
// - Once a window is evaluated, if it is periodic it will be incremented by a
//   period, otherwise removed
// - If an encounter is found, bring the end time forward to the time of the
//   encounter (so we don't continue to uselessly compute encounters after the
//   soonest known encounter)
pub fn find_next_encounter(
    model: &Model,
    orbit: &Orbit,
    entity: Entity,
    end_time: f64,
    ...
    // Evaluate entrance windows from soonest to latest until an encounter is found
    // or no windows remain Once an encounter is found, we'll have to evaluate
    // all the remaining windows that are not entirely after the time of the
    // discovered encounter Because an encounter with another object could
    // possibly happen sooner than the encounter we just found
    loop {
        windows.retain(|window| window.soonest_time() < end_time);
        #[allow(clippy::redundant_closure_for_method_calls)]
        // We would have to make Window public to fix this
        ...
        if let Some(encounter_time) =
            solve_for_entrance(window.orbit(), window.other_orbit(), from, to)?
        {
            // Add a minimum time as another encounter could be calculated as being eg 0.01
            // seconds later if eg an entity exits an SOI and then an 'entrance'
            // is calculated to be very shortly after
            if encounter_time > start_time + MIN_TIME_BEFORE_ENCOUNTER {
                soonest_encounter = Some(Encounter::new(
