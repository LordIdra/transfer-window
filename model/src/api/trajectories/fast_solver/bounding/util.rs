...
// This is harder than it first appears, because for example the range 5.9 to
// 5.8 contains the angle 1.4 We can work around this by considering both cases,
// 5.9 to 5.8 (out of order) and 5.8 to 5.9 (in order) Then check if the in
// order case contains the minimum If so, that's our solution. If not, the other
...
// Returns the (smallest) distance from the first angle to the second, ie
...
// Assuming we've already found a stationary point on a periodic function with 1
// minimum and 1 maximum, we can find the other by creating a range that just
// about excludes the known stationary point Should only be used on ellipse SDFs
pub fn find_other_stationary_point(
    distance_function: impl Fn(f64) -> f64,
    known_stationary_point_theta: f64,
    ...
    let derivative =
        |theta: f64| (distance_function(theta + 0.0001) - distance_function(theta)) / 0.0001;
