...
/// Returns a function that acts as a signed distance function in terms of an
/// angle on orbit A Negative when `orbit_a` is OUTSIDE `orbit_b`
pub fn make_sdf<'a>(orbit_a: &'a Orbit, orbit_b: &'a Orbit) -> impl Fn(f64) -> f64 + 'a {
    ...
    move |theta: f64| -> f64 {
        ...
        let point = orbit_a.position_from_theta(theta);
