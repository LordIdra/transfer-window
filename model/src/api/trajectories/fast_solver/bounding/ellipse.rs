...
use transfer_window_common::numerical_methods::laguerre::laguerre_to_find_stationary_point;

use super::sdf::make_sdf;
use super::util::find_other_stationary_point;
use super::window::Window;
use crate::api::trajectories::fast_solver::bounding::util::{
    angle_window_to_time_window, angular_distance, make_range_containing,
};
...
fn find_min_max_signed_distance(
    sdf: &impl Fn(f64) -> f64,
    argument_of_apoapsis: f64,
) -> Result<(f64, f64), &'static str> {
    #[cfg(feature = "profiling")]
    let _span = tracy_client::span!("Min max signed distance");
    let theta_1 =
        laguerre_to_find_stationary_point(&sdf, argument_of_apoapsis, 1.0e-4, 1.0e-6, 256);
    if theta_1.is_none() {
        return Err("Failed to converge while solving for ellipse bounds");
    }
    let theta_1 = theta_1.unwrap();
...
impl<'a> BounderData<'a> {
    ...
    fn no_bounds(self) -> Vec<Window<'a>> {
        ...
        self.make_segmented_window(
            true,
            self.start_time,
            self.start_time + self.orbit.period().unwrap(),
            16,
        )
    }
    ...
    fn one_bound_inner(self, sdf: impl Fn(f64) -> f64) -> Result<Vec<Window<'a>>, &'static str> {
        ...
        // The reason we segment here is that our solver relies on the assumption that
        // if there is a minimum, the derivative with respect to time is
        // positive at the start and negative at the end There are some cases
        // where this is not true, but splitting the window up into a few segments helps
    ...
    fn two_bounds(self, sdf: impl Fn(f64) -> f64) -> Result<Vec<Window<'a>>, &'static str> {
        ...
        let angle_bound_2 =
            make_range_containing(possible_tos[0], possible_tos[1], zero_intersections.1);
...
pub fn compute_ellipse_bound<'a>(
    orbit: &'a Orbit,
    sibling_orbit: &'a Orbit,
    sibling: Entity,
    start_time: f64,
