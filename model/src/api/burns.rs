...
impl Model {
    fn get_rocket_equation_function_at_end_of_trajectory(&self, entity: Entity) -> RocketEquationFunction {
        if let Some(burn) = self.get_path_component(entity).get_final_burn() {
    ...
    pub fn delete_segments_after_time_and_recompute_trajectory(&mut self, entity: Entity, time: f64) {
        let trajectory_component = self.get_path_component_mut(entity);
    ...
    pub fn create_burn(&mut self, entity: Entity, time: f64) {
        ...
        let parent = trajectory_component.get_end_segment().parent();
        let tangent = trajectory_component.get_end_segment().end_velocity().normalize();
        let start_position = trajectory_component.get_end_segment().end_position();
        let start_velocity = trajectory_component.get_end_segment().end_velocity();
        ...
        self.get_path_component_mut(entity).add_segment(Segment::Burn(burn));
        self.get_path_component_mut(entity).add_segment(Segment::Orbit(orbit));
        self.predict(entity, 1.0e10, SEGMENTS_TO_PREDICT);
    }

    pub fn adjust_burn(&mut self, entity: Entity, time: f64, amount: DVec2) {
        let trajectory_component = self.get_path_component_mut(entity);
        let end_time = trajectory_component.get_last_segment_at_time(time).end_time();
        trajectory_component.remove_segments_after(end_time);
        trajectory_component.get_end_segment_mut()
            .as_burn_mut()
        ...
        let end_time = self.get_path_component_mut(entity).get_last_segment_at_time(time).end_time();
