use crate::components::vessel_component::docking::{
    DockedVessel, DockingPortLocation, ResourceTransferDirection,
};
...
fn update_fuel_transfer(
    ...
    let amount = f64::min(
        f64::min(
            dt * model.time_step().time_step() * transfer.rate(),
            from_fuel,
        ),
        remaining_space,
    );
    ...
    if amount.abs() < 1.0e-3 {
        model
            .vessel_component_mut(station_entity)
            .docking_port_mut(location)
            .docked_vessel_mut()
            .stop_fuel_transfer();
    } else {
        model
...
fn update_torpedo_transfer(
    ...
    model
        .docking_port_mut(station_entity, location)
        .docked_vessel_mut()
        .step_torpedo_transfer(simulation_dt);
    transfer = transfer.step(simulation_dt);

    loop {
        if model.vessel_component(from).is_torpedoes_empty()
            || model.vessel_component(to).is_torpedoes_full()
        {
            model
                .docking_port_mut(station_entity, location)
                .docked_vessel_mut()
        ...
        model
            .docking_port_mut(station_entity, location)
            .docked_vessel_mut()
            .step_torpedo_transfer(-transfer.interval());
        transfer = transfer.step(-transfer.interval());
    }
}

fn update_docking_port(
    model: &mut Model,
    station_entity: Entity,
    location: DockingPortLocation,
    docked_vessel: &DockedVessel,
