...
impl FireTorpedoEvent {
    pub fn new(model: &mut Model, fire_from: Entity, time: f64) -> Self {
        let mut vessel_component =
            VesselClass::Torpedo.build(model.vessel_component(fire_from).faction());
        ...
        let rocket_equation_function =
            RocketEquationFunction::from_vessel_component(&vessel_component);
        let orbit = Orbit::new(
            fire_from_orbit.parent(),
            rocket_equation_function.mass(),
            parent_mass,
            point_at_time.position(),
            point_at_time.velocity(),
            time,
        );

        let ghost = model.allocate(
    ...
    /// # Panics
    /// Panics if the weapon slot requested does not in fact contain a weapon or
    /// is not a torpedo
    ...
    pub fn can_create_ever(model: &Model, entity: Entity) -> bool {
        ...
        vessel_component.has_torpedo_storage() && vessel_component.has_torpedo_launcher()
    }
