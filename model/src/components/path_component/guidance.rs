...
fn proportional_guidance_acceleration(
    absolute_position: DVec2,
    target_absolute_position: DVec2,
    absolute_velocity: DVec2,
    target_absolute_velocity: DVec2,
) -> DVec2 {
    let displacement = absolute_position - target_absolute_position;
    let closing_speed = -(absolute_velocity - target_absolute_velocity).dot(&displacement)
        / displacement.magnitude();

    let future_displacement = ((absolute_position + absolute_velocity * LINE_OF_SIGHT_RATE_DELTA)
        - (target_absolute_position + target_absolute_velocity * LINE_OF_SIGHT_RATE_DELTA))
        / LINE_OF_SIGHT_RATE_DELTA;
    let line_of_sight_rate = (f64::atan2(displacement.y, displacement.x)
        - f64::atan2(future_displacement.y, future_displacement.x))
        / LINE_OF_SIGHT_RATE_DELTA;
...
fn compute_guidance_points(
    ...
    target: Entity,
    faction: Faction,
    start_rocket_equation_function: &RocketEquationFunction,
    ...
    loop {
        ...
        let distance_at_delta_time = |delta_time: f64| {
            // Mass is technically not correct, but the difference is almost certainly
            // negligible, and it's not really important anyway
            let point = last.next(delta_time, rocket_equation_function.mass());
            (model.absolute_position_at_time(parent, point.time(), Some(faction))
                + point.position()
                - model.absolute_position_at_time(target, point.time(), Some(faction)))
            .magnitude()
        };
        let distance_prime_at_delta_time = |delta_time: f64| {
            (distance_at_delta_time(delta_time + DISTANCE_DERIVATIVE_DELTA)
                - distance_at_delta_time(delta_time))
                / DISTANCE_DERIVATIVE_DELTA
        };
        if distance_prime_at_delta_time(0.0).is_sign_negative()
            && distance_prime_at_delta_time(GUIDANCE_TIME_STEP).is_sign_positive()
        {
            // Distance derivative sign flips, so we have a minimum distance within
            // GUIDANCE_TIME_STEP
            match itp(&distance_prime_at_delta_time, 0.0, GUIDANCE_TIME_STEP) {
                ...
                Ok(intercept_delta_time) => {
                    ...
                    if will_intercept(intercept_distance) {
                        ...
                        points
                            .push(last.next(intercept_delta_time, rocket_equation_function.mass()));
                        return (true, points);
                    }
                }
            }
        ...
        let absolute_position =
            model.absolute_position_at_time(parent, time, Some(faction)) + last.position();
        let absolute_velocity =
            model.absolute_velocity_at_time(parent, time, Some(faction)) + last.velocity();
        ...
        let requested_acceleration = proportional_guidance_acceleration(
            absolute_position,
            target_absolute_position,
            absolute_velocity,
            target_absolute_velocity,
        );

        // Make sure guidance acceleration does not exceed max acceleration
        let actual_acceleration =
            if requested_acceleration.magnitude() < rocket_equation_function.acceleration() {
        ...
        points.push(last.next_with_new_acceleration_and_dv(
            GUIDANCE_TIME_STEP,
            mass,
            actual_acceleration,
        ));
...
impl Guidance {
    ...
    pub fn new(
        ...
        let start_point = GuidancePoint::new(
            parent_mass,
        ...
        let (will_intercept, points) = compute_guidance_points(
            model,
            parent,
            target,
            faction,
    ...
    pub fn rocket_equation_function_at_time(&self, time: f64) -> RocketEquationFunction {
        // Slightly annoying workaround to make sure that if the guidance expends all
        // our DV, there won't be a panic
