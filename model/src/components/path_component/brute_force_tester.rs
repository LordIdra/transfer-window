...
impl BruteForceTester {
    pub fn new(
        ...
        position: DVec2,
        velocity: DVec2,
        acceleration_from_time: impl Fn(f64) -> DVec2 + 'static,
        time_step: f64,
    ) -> Self {
    ...
    fn step(&mut self, dt: f64) {
        let gravity_acceleration =
            -self.position.normalize() * GRAVITATIONAL_CONSTANT * self.parent_mass
...
mod test {
    ...
    fn test_constant_acceleration() {
        ...
        let expected_position = vec2(
            0.5 * constant_acceleration.magnitude() * duration.powi(2),
            0.0,
        );
