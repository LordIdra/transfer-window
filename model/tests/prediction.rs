...
use transfer_window_model::{components::{name_component::NameComponent, orbitable_component::{OrbitableComponent, OrbitableComponentPhysics}, path_component::{burn::{rocket_equation_function::RocketEquationFunction, Burn}, orbit::Orbit, segment::Segment, PathComponent}, vessel_component::{VesselClass, VesselComponent}}, storage::entity_builder::EntityBuilder, Model, SEGMENTS_TO_PREDICT};
...
fn test_prediction() {
    ...
    let orbitable_component = OrbitableComponent::new(sun_mass, 1.0, OrbitableComponentPhysics::Stationary(vec2(0.0, 0.0)));
    ...
    let earth = model.allocate(EntityBuilder::default()
        ...
        .with_orbitable_component(OrbitableComponent::new(earth_mass, 1.0, OrbitableComponentPhysics::Orbit(orbit))));
    ...
    let _moon = model.allocate(EntityBuilder::default()
        ...
        .with_orbitable_component(OrbitableComponent::new(moon_mass, 1.0, OrbitableComponentPhysics::Orbit(orbit))));
...
fn test_prediction_with_burn() {
    ...
    let sun = model.allocate(EntityBuilder::default()
        .with_name_component(NameComponent::new("Sun".to_string()))
        .with_orbitable_component(OrbitableComponent::new(sun_mass, 1.0, OrbitableComponentPhysics::Stationary(vec2(0.0, 0.0)))));
