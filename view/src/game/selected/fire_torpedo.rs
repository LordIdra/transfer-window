...
use crate::game::util::compute_adjust_fire_torpedo_arrow_position;
use crate::game::View;

fn compute_drag_adjustment_amount(
    view: &View,
    entity: Entity,
    time: f64,
    ...
    burn_adjustment_amount(arrow_to_mouse.dot(&event_to_arrow_unit))
        * direction.vector()
        * view.camera.zoom().powi(2)
}
...
pub fn update_adjustment(view: &View, pointer: &PointerState) {
    ...
    } = &view.selected
    {
        if state.is_dragging() {
            // We don't check primary_released() because this does not trigger if mouse is
    ...
    } = view.selected.clone()
    {
        if let Some(mouse_position) = pointer.latest_pos() {
            ...
            if let Some(amount) =
                view.model.calculate_burn_dv(event.ghost(), event.burn_time(), amount)
            {
                view.add_model_event(ModelEvent::AdjustFireTorpedo {
                    entity,
                    time,
                    amount,
                });
