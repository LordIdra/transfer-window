...
pub fn tessellate(
    ...
    while i < points.len() - 2 {
        ...
        if area / min_distance > TESSELLATION_THRESHOLD {
            ...
            points.insert(
                i + 1,
                (new_time_1, absolute_parent_position + new_position_1),
            );
            points.insert(
                i + 3,
                (new_time_2, absolute_parent_position + new_position_2),
            );
        } else {
            i += 1;
        }
...
fn find_initial_points(guidance: &Guidance, absolute_parent_position: DVec2) -> Vec<(f64, DVec2)> {
    ...
    for i in 0..=INITIAL_POINT_COUNT {
        ...
        initial_points.push((
            time,
            absolute_parent_position + guidance.point_at_time(time).position(),
        ));
    }
