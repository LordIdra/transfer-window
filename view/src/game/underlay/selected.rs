...
use crate::game::selected::segment_point::{self, SELECT_DISTANCE};
use crate::game::selected::{burn, fire_torpedo, Selected};
use crate::game::View;
...
pub fn update(view: &View) {
    ...
    if pointer.primary_clicked() && !view.pointer_over_ui && !view.pointer_over_icon {
        ...
        if let Some(latest_window) = pointer.latest_pos() {
            ...
            } else if let Some((entity, time)) = view.model.closest_guidance_point(
                latest_world,
                select_distance,
                Some(Faction::Player),
            ) {
