...
fn get_time_of_next_periapsis(_model: &Model, orbit: &Orbit) -> Option<f64> {
    let periapsis_time = if orbit.is_ellipse() {
        let mut periapsis_time = orbit.get_first_periapsis_time();
        while periapsis_time <= orbit.get_current_point().get_time() {
            periapsis_time += orbit.get_period().unwrap();
        }
        periapsis_time
    } else {
        orbit.get_first_periapsis_time()
    };

    if periapsis_time > orbit.get_current_point().get_time() && periapsis_time < orbit.get_end_point().get_time() {
        Some(periapsis_time)
    } else {
        None
    }
