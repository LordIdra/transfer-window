...
fn draw(view: &mut Scene, model: &Model, context: &Context, events: &mut Vec<Event>, entity: Entity) {
    ...
    Window::new("Vessel menu")
        .title_bar(false)
        .resizable(false)
        .anchor(Align2::LEFT_BOTTOM, window_position.to_vec2())
        .show(context, |ui| {
            let can_target = if let Some(focus) = view.camera.get_focus() {
                (focus != entity) && model.try_get_vessel_component(focus).is_some()
            } else {
                false
            };
...
pub fn update(view: &mut Scene, model: &Model, context: &Context, events: &mut Vec<Event>, is_mouse_over_any_icon: bool) {
    ...
    let _span = tracy_client::span!("Update vessel menu");
    ...
    if !is_mouse_over_ui_element {
        ...
        if view.vessel_menu.is_some() && pointer.primary_clicked() {
            view.vessel_menu = None;
        }
    }

    if let Some(entity) = view.vessel_menu {
