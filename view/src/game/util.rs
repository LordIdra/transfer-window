...
use super::selected::util::BurnAdjustDirection;
use super::View;
...
pub fn add_textured_square_facing(
    vertices: &mut Vec<f32>,
    position: DVec2,
    radius: f64,
    color: f32,
...
pub fn format_time(time: f64) -> String {
    let start_string = if time.is_sign_positive() {
        "".to_string()
    } else {
        "-".to_string()
    };
    ...
    if years_quotient != 0.0 {
        start_string
            + years_quotient.to_string().as_str()
            + "y"
            + days_quotient.to_string().as_str()
            + "d"
            + hours_quotient.to_string().as_str()
            + "h"
            + minutes_quotient.to_string().as_str()
            + "m"
        ...
        start_string + seconds.to_string().as_str() + "s"
    }
}
...
pub fn compute_burn_arrow_position(
    view: &View,
    entity: Entity,
    time: f64,
...
pub fn compute_adjust_fire_torpedo_arrow_position(
    view: &View,
    entity: Entity,
    time: f64,
    direction: BurnAdjustDirection,
