...
impl ObjectRenderer {
    pub fn new(gl: Arc<Context>, texture: Texture) -> Self {
        let program = ShaderProgram::new(gl.clone(), include_str!("../../../resources/shaders/planet.vert"), include_str!("../../../resources/shaders/planet.frag"));
        let vertex_array_object = VertexArrayObject::new(gl, vec![
            VertexAttribute { index: 0, count: 2 }, // x
            VertexAttribute { index: 1, count: 2 }, // y
            VertexAttribute { index: 2, count: 2 }, // texture coordinates
        ]);
        let vertices = vec![];
        let rotation = None;
    ...
    pub fn render(&mut self, zoom_matrix: Mat3, translation_matrices: (Mat3, Mat3)) {
        ...
        self.program.uniform_float("rotation", self.rotation.expect("Rotation not set"));
