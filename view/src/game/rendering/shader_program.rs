...
use eframe::glow;
use glow::{Context, HasContext, NativeUniformLocation, Program, FRAGMENT_SHADER, VERTEX_SHADER};
...
impl ShaderProgram {
    ...
    pub fn uniform_bool(&self, gl: &Arc<Context>, name: &str, v: bool) {
        self.use_program(gl);
        unsafe {
            gl.uniform_1_i32(Some(&Self::location(self, gl, name)), v as i32);
        }
    }
    ...
    pub fn uniform_int(&self, gl: &Arc<Context>, name: &str, v: i32) {
        self.use_program(gl);
        unsafe {
            gl.uniform_1_i32(Some(&Self::location(self, gl, name)), v);
        }
    }
    ...
    pub fn uniform_float(&self, gl: &Arc<Context>, name: &str, v: f32) {
        self.use_program(gl);
        unsafe {
            gl.uniform_1_f32(Some(&Self::location(self, gl, name)), v);
        }
    }
    ...
    pub fn uniform_vec2(&self, gl: &Arc<Context>, name: &str, x: f32, y: f32) {
        self.use_program(gl);
        unsafe {
            gl.uniform_2_f32(Some(&Self::location(self, gl, name)), x, y);
        }
    }
    ...
    pub fn uniform_mat3(&self, gl: &Arc<Context>, name: &str, v: &[f32]) {
        self.use_program(gl);
        unsafe {
            gl.uniform_matrix_3_f32_slice(Some(&Self::location(self, gl, name)), false, v);
        }
    }
