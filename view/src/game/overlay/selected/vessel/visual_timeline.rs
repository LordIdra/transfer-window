...
use crate::game::overlay::widgets::util::advance_cursor_to;
use crate::game::selected::util::BurnState;
use crate::game::selected::Selected;
use crate::game::util::{format_distance, format_time, ApproachType, ApsisType};
use crate::game::View;
...
impl VisualTimelineEvent {
    ...
    pub fn name(&self, view: &View) -> String {
        match self {
            ...
            VisualTimelineEvent::Apsis {
                type_, distance, ..
            } => match type_ {
                ApsisType::Periapsis => format!("Periapsis - {}", format_distance(*distance)),
                ApsisType::Apoapsis => format!("Apoapsis - {}", format_distance(*distance)),
            },
            VisualTimelineEvent::Approach { distance, .. } => {
                format!("Approach - {}", format_distance(*distance))
            }
            VisualTimelineEvent::Encounter {
                type_, from, to, ..
            } => match type_ {
                EncounterType::Entrance => {
                    format!("{} Entrance", view.model.name_component(*to).name())
                }
    ...
    pub fn selected(&self, entity: Entity) -> Option<Selected> {
        match self {
            VisualTimelineEvent::TimelineEvent(event) => Some(match event {
                TimelineEvent::Intercept(intercept) => Selected::Intercept {
                    entity,
                    time: intercept.time(),
                },
                TimelineEvent::FireTorpedo(fire_torpedo) => Selected::FireTorpedo {
                    entity,
                    time: fire_torpedo.time(),
                    state: BurnState::Selected,
                },
                TimelineEvent::Burn(burn) => Selected::Burn {
                    entity,
                    time: burn.time(),
                    state: BurnState::Selected,
                },
                TimelineEvent::EnableGuidance(enable_guidance) => Selected::EnableGuidance {
                    entity,
                    time: enable_guidance.time(),
                },
            }),
            VisualTimelineEvent::Apsis {
                type_,
            ...
            VisualTimelineEvent::Approach {
                type_,
                target,
                time,
                distance: _,
            } => Some(Selected::Approach {
                type_: *type_,
                entity,
                target: *target,
                time: *time,
    ...
    pub fn is_selected(&self, view: &View, other_entity: Entity) -> bool {
        ...
        match view.selected {
            ...
            Selected::Apsis { type_, time, .. } => {
                match self {
                    ...
                    VisualTimelineEvent::Apsis {
                        ...
                        time: other_time,
                        ..
                    } => type_ == *other_type && (time - *other_time).abs() < 1.0,
                    _ => false,
                }
            }
            Selected::Approach { type_, .. } => match self {
                VisualTimelineEvent::Approach {
                    type_: other_type, ..
                } => type_ == *other_type,
                _ => false,
            ...
            Selected::Burn { time, .. } => match self {
                VisualTimelineEvent::TimelineEvent(TimelineEvent::Burn(event)) => {
                    event.time() == time
                }
                _ => false,
            },
            Selected::FireTorpedo { time, .. } => match self {
                VisualTimelineEvent::TimelineEvent(TimelineEvent::FireTorpedo(event)) => {
                    event.time() == time
                }
                _ => false,
            },
            Selected::EnableGuidance { time, .. } => match self {
                VisualTimelineEvent::TimelineEvent(TimelineEvent::EnableGuidance(event)) => {
                    event.time() == time
                }
                _ => false,
            },
...
fn generate_apoapsis_periapsis(view: &View, entity: Entity, events: &mut Vec<VisualTimelineEvent>) {
    for orbit in view.model.future_orbits(entity, Some(Faction::Player)) {
        if let Some(time) = orbit.next_periapsis_time() {
            ...
            events.push(VisualTimelineEvent::Apsis {
                type_: ApsisType::Periapsis,
                time,
                distance,
            });
        }
...
fn generate_encounters(view: &View, entity: Entity, events: &mut Vec<VisualTimelineEvent>) {
    for encounter in view.model.future_encounters(entity, Some(Faction::Player)) {
        events.push(VisualTimelineEvent::Encounter {
            type_: encounter.encounter_type(),
            time: encounter.time(),
            from: encounter.from(),
            to: encounter.to(),
        });
...
fn generate_burn_guidance_end(view: &View, entity: Entity, events: &mut Vec<VisualTimelineEvent>) {
    if let Segment::Burn(burn) = view.model.path_component(entity).current_segment() {
        events.push(VisualTimelineEvent::BurnEnd {
    ...
    if let Segment::Guidance(guidance) = view.model.path_component(entity).current_segment() {
        if !guidance.will_intercept() {
            events.push(VisualTimelineEvent::GuidanceEnd {
                time: guidance.end_point().time(),
            });
        }
    }
}

pub fn draw_visual_timeline(
    view: &View,
    ui: &mut Ui,
    entity: Entity,
    center_time: f64,
    draw_center_time_point: bool,
) {
    let mut events = vec![];
...
fn draw_event(
    ...
    frame.content_ui.horizontal(|ui| {
        ...
        let image = CustomImage::new(view, event.icon(), 20.0).with_padding(event.padding());
        ui.add(image);
    ...
    // Annoying workaround because allocate_space only uses hover sense for some
    // reason
