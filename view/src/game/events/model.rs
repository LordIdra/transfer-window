...
impl View {
    pub fn save_game(&mut self, name: &str) {
        ...
        let Ok(serialized) = serialized else {
            error!(
                "Failed to handle save_game; error while serializing: {}",
    ...
    pub fn create_fire_torpedo(&mut self, entity: Entity, time: f64) {
        ...
        let event =
            TimelineEvent::FireTorpedo(FireTorpedoEvent::new(&mut self.model, entity, time));
        self.model.add_event(entity, event);
    }
    ...
    pub fn enable_torpedo_guidance(&mut self, entity: Entity, time: f64) {
        ...
        let event =
            TimelineEvent::EnableGuidance(EnableGuidanceEvent::new(&mut self.model, entity, time));
        self.model.add_event(entity, event);
    }
    ...
    pub fn start_torpedo_transfer(
        ...
        self.model
            .docking_port_mut(station, location)
            .docked_vessel_mut()
            .start_torpedo_transfer(direction);
    }
