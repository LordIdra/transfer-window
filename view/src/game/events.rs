...
use transfer_window_model::components::vessel_component::docking::{
    DockingPortLocation, ResourceTransferDirection,
};
...
use super::debug::DebugWindowTab;
use super::overlay::vessel_editor::VesselEditor;
use super::selected::Selected;
use super::View;
use crate::controller_events::ControllerEvent;
use crate::game::overlay::dialogue::Dialogue;
use crate::game::overlay::objectives::Objective;
...
pub enum ModelEvent {
    ...
    SetFuelTank {
        entity: Entity,
        type_: Option<FuelTankType>,
    },
    SetEngine {
        entity: Entity,
        type_: Option<EngineType>,
    },
    SetTorpedoStorage {
        entity: Entity,
        type_: Option<TorpedoStorageType>,
    },
    SetTorpedoLauncher {
        entity: Entity,
        type_: Option<TorpedoLauncherType>,
    },
    CreateFireTorpedo {
        entity: Entity,
        time: f64,
    ...
    pub(crate) fn handle_events(&mut self) {
        ...
        while let Some(event) = model_events.pop() {
            ...
            match event {
                ...
                ModelEvent::SetTorpedoStorage { entity, type_ } => {
                    self.set_torpedo_storage(entity, type_)
                }
                ModelEvent::SetTorpedoLauncher { entity, type_ } => {
                    self.set_torpedo_launcher(entity, type_)
                }
                ModelEvent::CreateFireTorpedo { entity, time } => {
                    self.create_fire_torpedo(entity, time)
                }
                ModelEvent::AdjustFireTorpedo {
                    entity,
                    time,
                    amount,
                } => self.adjust_fire_torpedo(entity, time, amount),
                ModelEvent::CancelLastTimelineEvent { entity } => self.cancel_last_event(entity),
                ModelEvent::CreateGuidance { entity, time } => {
                    self.enable_torpedo_guidance(entity, time)
                }
                ...
                ModelEvent::StartFuelTransfer {
                    station,
                    location,
                    direction,
                } => self.start_fuel_transfer(station, location, direction),
                ModelEvent::StopFuelTransfer { station, location } => {
                    self.stop_fuel_transfer(station, location)
                }
                ModelEvent::StartTorpedoTransfer {
                    station,
                    location,
                    direction,
                } => self.start_torpedo_transfer(station, location, direction),
                ModelEvent::StopTorpedoTransfer { station, location } => {
                    self.stop_torpedo_transfer(station, location)
                }
            }
        }
        ...
        while let Some(event) = view_events.pop() {
            ...
            match event {
                ...
                ViewEvent::SetDebugWindowOpen(debug_window_open) => {
                    self.debug_window_open = debug_window_open
                }
                ViewEvent::SetDebugWindowTab(debug_window_tab) => {
                    self.debug_window_tab = debug_window_tab
                }
                ...
                ViewEvent::StartObjective(objective) => {
