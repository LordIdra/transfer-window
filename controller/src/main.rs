...
use crate::event_handler::{create_burn, debug_add_entity, decrease_time_step_level, increase_time_step_level, load_game, new_game, quit, save_game, start_warp, toggle_paused};
...
impl App for Controller {
    fn update(&mut self, context: &Context, frame: &mut Frame) {
        ...
        let events = match &mut self.view {
            View::GameScene(scene) => scene.update(self.model.as_ref().unwrap(), context),
