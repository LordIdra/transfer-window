...
use eframe::egui::{Context, Key, ViewportBuilder, ViewportCommand};
use eframe::glow::{self, HasContext, RENDERER, SHADING_LANGUAGE_VERSION, VERSION};
use eframe::{run_native, App, CreationContext, Frame, NativeOptions};
use event_handler::{finish_level, load_game, new_game, quit};
use log::{debug, info};
use sysinfo::System;
...
use transfer_window_view::controller_events::ControllerEvent;
use transfer_window_view::resources::Resources;
use transfer_window_view::{menu, Scene};
...
struct Controller {
    ...
    scene: Scene,
    last_frame: Instant,
    load_menu: bool, /* menu loading must occur RIGHT AT THE BEGINNING of a frame before any
                      * updates */
}

fn log_gl_info(gl: &Arc<glow::Context>) {
    info!("OpenGL version: {}", unsafe {
        gl.get_parameter_string(VERSION)
    });
    info!("OpenGL renderer: {}", unsafe {
        gl.get_parameter_string(RENDERER)
    });
    info!("GLSL version: {}", unsafe {
        gl.get_parameter_string(SHADING_LANGUAGE_VERSION)
    });
}
...
impl Controller {
    ...
    pub fn init(creation_context: &CreationContext) -> Result<Box<dyn App>, DynError> {
        ...
        let view = Scene::Menu(menu::View::new(
            resources.clone(),
    ...
    fn handle_events(&mut self, mut events: Vec<ControllerEvent>, context: &Context) {
        ...
        while let Some(event) = events.pop() {
            ...
            match event {
                ...
                ControllerEvent::NewGame { story_builder } => {
                    new_game(self, context, &*story_builder)
                }
...
impl App for Controller {
    fn update(&mut self, context: &Context, frame: &mut Frame) {
        ...
        if self.load_menu {
            self.scene = Scene::Menu(menu::View::new(
